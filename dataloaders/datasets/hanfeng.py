...
import dataloaders.augmentation as aug


class HanFengDataSet(Dataset):
    ...
    def transform_tr(self, sample):
        composed_transforms = transforms.Compose([
            # aug.RandomCrop((512, 640)),
            aug.RandomScaleCrop(self.cfg['base_size'], self.cfg['crop_size']),
            aug.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
    ...
    def transform_val(self, sample):
        ...
        composed_transforms = transforms.Compose([
            # aug.FixScaleCrop(crop_size=self.crop_size),
            aug.FixedResize(640, 512),
            aug.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
            aug.ToTensor()])
...
if __name__ == '__main__':
    ...
    config = get_config('hanfeng/deeplab-resnet-hanfeng.json')

    CamVid_Dataset = HanFengDataSet(config, split='val')
